...
simulator.Initialize(init_vertices, elements, 1e3, 0.5e6, 0.3, dirichilet_boundary)
...
def set_force(a):
    for i in range(Y+1):
        for j in range(X+1):
            mu = 0.5
            simulator.external_acceleration[(i*(X+1) + j)*2] = torch.tensor([0.0, 0.0, a * torch.exp(-mu * torch.abs(torch.tensor(i - Y)))])
            simulator.external_acceleration[(i*(X+1) + j)*2+1] = torch.tensor([0.0, 0.0, a * torch.exp(-mu * torch.abs(torch.tensor(i - Y)))])
...
for f in tqdm(range(300)):
    ...
    set_force(torch.cos(torch.tensor(f) * 0.1) * 20)
    simulatorController.Forward(0.02)
    position_np = simulator.position.detach().cpu().numpy()
