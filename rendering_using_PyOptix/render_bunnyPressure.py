...
# m_plastic["VarFloat3"]["refraction_index"] = [0.0, 0.0, 0.0]
...
def update(rt: NpOptiX)->None:
    ...
    z = 1.5 - 1.3 * float(params.n)/250
    rt.update_mesh("roof", pos = np.array([[10,10,z],[10,-10,z],[-10,-10,z],[-10,10,z]]))
...
optix.set_mesh("ground",pos = np.array([[0,1000,0],[-1000,-1000,0],[1000,-1000,0]]),c = make_color(0.6),mat="plastic",make_normals=True,faces=np.array([[0,1,2]]))
optix.set_mesh("roof",pos=np.array([[10,10,1.5],[10,-10,1.5],[-10,-10,1.5],[-10,10,1.5]]), c = make_color_2d(0.6,alpha=0.5), mat = "glass",make_normals = True,faces=np.array([[0,1,3],[2,1,3]]))

optix.setup_camera("cam1", cam_type="Pinhole",
                   eye=[2.5,2.5, 0.2], target=[0, 0, 0.4], up=[0, 0, 1],
                   aperture_radius=0.0,
                   fov=35)
...
# optix.setup_light("ligh", pos=[15, 12, 10], color=[120,30,90], radius=2)
# optix.setup_light("lig", pos=[-15, 12, 10], color=[120,30,90], radius=2)
# optix.setup_light("ligqwe", pos=[15, -12, 10], color=[120,30,90], radius=2)
# optix.setup_light("ligq34grewvs", pos=[-15, -12, 10], color=[120,30,90], radius=2)
optix.setup_light("ligqfewd1", pos=[15, 12, 15], color=[120,30,90], radius=2)
optix.setup_light("lqwefght2", pos=[-15, 12,15], color=[120,30,90], radius=2)
optix.setup_light("lighqwefwet3", pos=[15, -12, 15], color=[120,30,90], radius=2)
optix.setup_light("lighqeqwft4", pos=[-15, -12, 15], color=[120,30,90], radius=2)
optix.setup_light("boauweufl", pos = [3,3,0.2], color = [120,30,90],radius = 0.2)
optix.setup_light("boauweufl", pos = [5,3,0.2], color = [120,30,90],radius = 0.2)
optix.setup_light("boauweufl", pos = [3,15,0.2], color = [120,30,90],radius = 0.2)
optix.start()

optix.encoder_start("bunnyPressure_03.mp4", params.fps * params.duration)
import time
n = 0
while n < 1000:
    n+= 1
